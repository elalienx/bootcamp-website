/* eslint-disable */
define(["require", "jquery"], function (b) {
  var c = b("jquery");
  var a = c.fn.tree;
  var d = function (f, e) {
    this.$element = c(f);
    this.options = c.extend({}, c.fn.tree.defaults, e);
    this.$element.on(
      "click",
      ".tree-item",
      c.proxy(function (g) {
  ...
  d.prototype = {
      ...
      this.options.dataSource.data(g.data(), function (i) {
        ...
        c.each(i.data, function (j, m) {
          ...
          if (m.type === "folder") {
            ...
            k.find(".tree-folder-header").data(m);
          } else {
            if (m.type === "item") {
    ...
    selectItem: function (g) {
      ...
      if (f.hasClass("tree-selected")) {
        ...
        f.find("i").removeClass("icon-ok").addClass("tree-dot");
      } else {
        f.addClass("tree-selected");
        f.find("i").removeClass("tree-dot").addClass("icon-ok");
        if (this.options.multiSelect) {
          h.push(f.data());
        }
      }
    ...
    selectFolder: function (i) {
      var g = c(i);
      var k = g.parent();
      var e = k.find(".tree-folder-content");
      var j = e.eq(0);
      var f, l, h;
      if (g.find(".icon-folder-close").length) {
        f = "opened";
        l = ".icon-folder-close";
        h = "icon-folder-open";
        j.show();
  ...
  c.fn.tree.Constructor = d;
  c.fn.tree.noConflict = function () {
    c.fn.tree = a;
    return this;
  };
});
